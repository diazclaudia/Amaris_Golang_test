...
func PublishData(id, points string, repoRead *read.RepositoryRead) {
	broker := NewBroker()

	subscriber := broker.Subscribe(id)
	go func() {
		for {
			select {
			case msg, ok := <-subscriber.Channel:
				...
				//updating the read database
				repoRead.Update(id, fmt.Sprintf("%v", msg))
